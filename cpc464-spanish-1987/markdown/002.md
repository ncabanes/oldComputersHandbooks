...
operaciones de forma condicional con la instrucción **IF ... THEN ...**
...
El programa debe permitirnos almacenar 100 nombres y números de teléfono.
...
No es imprescindible dar nombre a todos los programas, pero comprobará cuando
...
El programa ha almacenado la información que le hemos suministrado, pero no ha
...
```locobasic
47 PRINT "Pulse [INTRO] para terminar"
```
...
La línea 95 comprueba si **NOMBRE\$(x)** es la cadena vacía; si lo es, concluye la 
...
por el usuario. Escriba lo siguiente:
...
buscando, captado en la línea 130) está contenida en **NOMBRE\$(x)**. Si no lo 
está, **INSTR** genera el valor **0**, y entonces el programa salta a la línea 
180 para probar con el siguiente valor de **x**. Si el programa ha examinado 
sin éxito todos los elementos de la lista, continúa en la línea 190, donde 
informa de lo que ha ocurrido. En cambio, si encuentra el nombre, **INSTR** 
produce un valor mayor que **0**, lo que permite que el programa pase de la 
línea 150 a la 160 para escribir el nombre y el número de teléfono y luego 
terminar con la instrucción **END** de la línea 170.
...
forma: **si el=1**, invoca la subrutina de la línea 40; **si el=2**, invoca la
...
¡Mucho más limpio! Pero sigamos con el programa. Ahora queremos que cada vez
...
Si (**IF**) la longitud de **NOMBRE\$(x)** es mayor que **0**, es decir, si ya 
hay un dato en ese elemento de la lista, entonces (**THEN**) ir a la línea 180 
(que provoca el salto al siguiente elemento de la lista):
...
Para no tener que escribir los números de línea, ejecutaremos la orden
...
Cuando haya terminado de teclear la línea 410 y haya pulsado \[INTRO\],
pulse \[ESC\] para interrumpir la función **AUTO**.

Como tenemos una rutina más, tendremos que poner otro número de línea en la 
instrucción **ON el GOSUB** de la línea 100. Para ello vamos a corregir 
...
Observe la línea 370, en la que se escriben los valores de **NOMBRE\$(x)** y **TEL$(\x)**
...
Los ocho primeros (**#0** a **#7**) dirigen los datos de la pantalla \[bien a 
la pantalla completa, bien a alguna ventana (**WINDOW**)\].
...
entrada (**IN**) el fichero que tenemos en la cinta con el nombre de 
'**datos**'. Después leemos con **INPUT** (por el canal **#9**) los valores de 
las variables **NOMBRE\$(x)** y **TEL\$(x)**, de 1 a 100, y finalmente cerramos 
(**CLOSE**) el fichero y retornamos (**RETURN**) al menú.
...
Por fin hemos conseguido un programa que realiza todas las funciones previstas. 
Todo lo que nos queda ahora es ocuparnos de cómo presentar los resultados en la 
pantalla.
