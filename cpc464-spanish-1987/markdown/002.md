...
**NOMBRE$(27)**, donde **27** es el índice y **NOMBRE$** es lo que denominamos variable
...
En nuestro programa necesitamos dos listas, **NOMBRE$** y **TEL$**, cada una con
...
línea 130) está contenida en NOMBRE$(x). Si no lo está, INSTR genera el valor
...
### (=== Pendiente de seguir revisando desde aquí, pág 73 de 296 ===)

El listado es ahora:

```locobasic
10 REM gula de telefonos 20 DIM NOMBRE$(100)
30 DIM TEL$1100)
...
160 IF NOMBRE$(x)="" THEN 190 170 INPUT;" Telefono ";TEL$(x) 180 NEXT
190 PRINT "Fin de la entrada de datos"
200 RETURN
210 FOR x=1 TO 100
220 IF NOMBRE$(x)="" THEN 250 230 PRINT NOMBRE$(x);" ";TEL$(x)
240 NEXT
...
Cuando haya terminado de teclear la línea 410 y haya pulsado [INTRO] pulse
[ESC] para interrumpir la función AUTO.
...
NOMBRE$(x) y TEL$(x), de 1 a 100, y finalmente cerramos (CLOSE) el fichero y
...
190 FOR x=1 TO 100
200 CLS
210 IF LENINOMBRE$(x))>0 THEN 260
...
360 FOR x=1 TO 100
370 IF INSTR(NOMBRE$(x),BUSCAS)=0 THEN 400 
