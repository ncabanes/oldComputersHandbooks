...
Usted está obligado a incluir el mensaje de copyright en todas las
...
Dispositivo que facilita la interacción entre el usuario y el ordenador
(teclado, sonido, etc.)
...
Zona de memoria, en CP/M entre las posiciones &0000 y &0100, en la que
...
Procedimientos, operaciones u órdenes básicos en los que consiste Dr.
LOGO.
...
Una variación sobre un tema clásico. Para un jugador contra el
...
500 **PRINT**:**PRINT**\"Cada vez que aterriza, sube la alturade los
...
2970 **DATA**
255,230,255,240,255,240,270,240,255,250,270,250,270,250,270,260

2980 **DATA**
270,260,255,260,255,250,255,270,270,280,270,300,270,300,255,300

2990 **DATA**
255,310,255,330,255,330,270,330,270,330,270,310,270,310,255,310

3000 **DATA** 255,320,270,320

3010 **DATA**
310,355,310,375,350,355,335,355,335,355,335,365,335,365,350,365

3020 **DATA**
350,365,350,375,350,375,335,375,365,355,380,355,380,355,380,375

3030 **DATA**
380,375,365,375,380,365,365,365,410,355,410,375,410,365,395,365

3040 **DATA**
395,365,395,375,425,355,440,355,440,355,440,365,440,365,425,365

3050 **DATA**
425,365,425,375,425,375,440,375,455,375,455,355,455,355,470,355

3060 **DATA**
470,355,470,365,470,365,455,365,485,375,500,375,500,375,500,355

3070 **DATA**
515,375,515,355,515,355,530,355,530,355,530,375,530,375,515,375

3080 **DATA** 515,365,530,365

### Guante blanco

Llévese las joyas del señor conde. Muchos obstáculos con los que
tropezar, luces que encender, y cuidado con el perro. Para un jugador
contra el ordenador. Teclado o joystick.
...
40 **MODE** 0:**INK** 0,0:**BORDER** 0:**INK** 1,26:**INK** 2,15:**INK**
3,25

50 **INK** 4,14:**INK** 5,24,12:**INK** 6,0:**INK** 7,0:**INK**
8,0:**PAPER** #1, 7
...
280 i\$=UPPER\$(**INKEY**\$):**IF** i\$\<\>\"S\" **AND** i\$\<\>\"N\"
**THEN** 280
...
570 vent\$(1)=STRING\$(2,250):
vent\$(2)=**CHR**\$(251)+**CHR**\$(8)+**CHR**\$(10)+**CHR**\$(251)+**CHR**\$(8)+**CHR**\$(10)+**CHR**\$(251)

580 puerta\$(1)=STRING\$(2,252):
puerta\$(2)=**CHR**\$(253)+**CHR**\$(8)+**CHR**\$(10)+**CHR**\$(253)+**CHR**\$(8)+**CHR**\$(10)+**CHR**\$(253)
...
620
gol\$=**CHR**\$(246)+**CHR**\$(248)+**CHR**\$(247)+**CHR**\$(249)+**CHR**\$(252)+
**CHR**\$(253)+**CHR**\$(250)+**CHR**\$(251)+joya\$+obj\$+perro\$
...
810 **LOCATE** 9,1:**PRINT**\"PASE\":**LOCATE** 10,2:**PRINT**
**CHR**\$(213)**CHR**\$(212);
...
850 **PEN** 2:**PRINT**
**LEFT**\$(puerta\$(1),1);**LEFT**\$(puerta\$(2),1);\"Puertas\":**PRINT**

860 **PEN** 3:**PRINT** conm\$(1,0);conm\$(2,0);\" Luces
apagadas\":**PRINT**

870 **PEN** 3:**PRINT** conm\$(1,1);conm\$(2,1);\" Luces encendidas\"
:**PRINT**

880 **PEN** 4:**PRINT**
**LEFT**\$(vent\$(1),1);**LEFT**\$(vent\$(2),1);\" Ventanas\":**PRINT**

890 **PEN** 5:**PRINT** joya\$;\" Piedras preciosas\":**PRINT**

900 **PAPER** 1:**PEN** 0:**PRINT** obj\$;\" Obstaculos\":**PEN**
1:**PAPER** 0:**PRINT**
...
1280 **IF** luces(rm) **THEN** **INK** 7,10:**INK** 8,10 **ELSE**
**INK** 7,0:**INK** 8,0
...
1650 **IF** perro\>0 **THEN** perro=perro **MOD** 2+1:**IF** perro=2
**THEN** 2560
...
1740 **IF** luces(rm) **THEN** **INK** 7,10:**INK** 8,10 **ELSE**
**INK** 7,0:**INK** 8,0
...
1940 **IF** joyas=robado **THEN** **LOCATE** 8,7:**PRINT**\"todas
las\";**ELSE** **LOCATE** 8,7
...
2580 **IF** (perrox=xp **AND** perroy=yp) **OR** (perrox=xp+xf **AND**
perroy=yp+yf) **THEN** 2660
...
A\> (inductor de CP/M) 53

ABS 111

AFTER 111, 344

Almacenamiento de imágenes de pantalla en el segundo banco de memoria
317

already exists 208

Altavoces externos 27

Ampliaciones del sistema residentes 307

Amplificador 27

AMS 226

AMSDOS 84, 196, 197, 222
...
Aritmética sencilla 47

Arranque en frío de CP/M Plus 211

Array already dimensioned 289
...
Cabeceras de fichero en AMSDOS 198

Calidad de la grabación en cinta 195

CALL 114

Cambio de discos 197

canales de sonido 78
...
Caracteres definidos por el usuario 338

Caracteres especiales 109

Caracteres transparentes 361

Carga de LOGO 227

Carga de pantallas 201

Carga de programas 37

Carga de programas AMSOFT 38

Carga del disco a la memoria del ordenador 57
...
Códigos ampliados de control en BASIC 261

colocar el cursor (Locate) 67

Colores 60

Colores de CP/M Plus 215
...
Conexión del ordenador a la red 20

Conexión del ordenador a un monitor 21

Conexiones 298

Configuración de la impresora 302

Conjunto de caracteres gráficos 269

Consola en CP/M Plus 310
...
Copia de discos con CP/M 85

Copia de ficheros de disco a disco 205

Copia de ficheros de un disco a otro 184

Copia de ficheros entre disco y cinta 205

Copia de los discos maestros 182

Copia de programas de disco a disco 58
...
CP/M 2.2 (diferencias con CP/M Plus) 222

CP/M Plus 210

CPCE 430

Creación de un disco de trabajo de CP/M 185
...
Definición de una envolvente de tono 81

Definición de una envolvente de volumen 80

Definir caracteres 338
...
dibujar circunferencias 71

dibujar un punto (Plot) 69

dibujar una recta (Draw) 70

diferencia (Logo) 238

Digital Research 372
...
Direct command found 290

Directorio del disco 197
...
Discos de trabajo 182

discos maestros del sistema 182

Dispositivos de expansión 28

División 48

Division by zero 289
...
Duración del sonido 78

E

E (notación científica) 51
...
Envolvente de tono 79

Envolvente de volumen 79
...
Errores de disco en AMSDOS 291

Errores de lectura en cinta 192

ESC (tecla) 35

Esquemas del teclado 282
...
Eyección de un disco 31

F

Failed to load CP/M 209
...
Ficheros en disco con Dr.Logo 252

Ficheros en RAM 319

FIJA MAYS (tecla) 33

File already open 291
...
flechas del teclado 32

FN 131

FOR 45, 131

Formato de escritura 339

Formato IBM 306

Formato SISTEMA 305

Formato SÓLO DATOS 305

Formatos literales 158

Formatos numéricos 157
...
Frecuencias de sonido 285

fs 245

Funciones gráficas 358

G

Garantía 373

Gestor de bancos de memoria 91, 317
...
Grabación de un programa en disco 57

Gráficos (ejemplo) 358

Gráficos con el segundo banco de 64K de RAM 364

Gráficos de tortuga 243, 247
...
impresora desde Logo 242

Impresoras 302

Impresoras en CP/M Plus 217

Improper argument 289

inductor A\> 53

Inicialización de discos 52
...
Interfaz serie en CP/M Plus 217

Interrupciones 267, 344

INTRO (tecla) 32

Introducción a CP/M Plus 183

Introducción a LOGO 227

Invalid direct command 289
...
Joystick en Dr.Logo 254

Joysticks 26, 284, 304

Juego de caracteres 269, 333

Juego de caracteres de CP / M Plus 314
...
Léxico inglés-castellano 375

Licencia 372

Limpieza del ordenador 25

Line does not exist 289

LINE INPUT 140

Line too long 290

Líneas discontinuas 359
...
mayúsculas y minúsculas 65

memberp 234

Memoria 308

memoria utilizada por Dr. LOGO 231

MEMORY 143

Memory ful1 289

Mensajes de error de AMSDOS 208

Mensajes de error de BASIC 289
...
Nombres de fichero con dos unidades de disco 199

Nombres y tipos de ficheros en AMSDOS 198
...
Notación científica 51

Notas de sonido y periodos de tono 285
...
Números de las teclas 284

Números de tinta 61

O

ON \<expresión\> GOSUB 149

ON \<expresión\> GOTO 150
...
órdenes externas 307

Ordenes transitorias de CP/M Plus 214

origen de coordenadas (Origin) 73
...
palabras clave (introducción) 39

Palabras clave del BASIC de AMSTRAD 109

Palabras reservadas de BASIC 293
...
Posiciones del cursor en BASIC 261
...
Prioridad en operaciones aritméticas 50

Problemas en el funcionamiento del sistema 25

Procedimientos en Dr. LOGO 229
...
Protección contra escritura de un disco 29

protegidos, ficheros 58
...
Reinicializar el ordenador 64

RELEASE 160, 354

release (Logo) 256

rellenar zonas de pantalla (Fill) 74
...
signo de interrogación 40

SIN 165

sin (Logo) 237

sincronizar barrido (Frame) 68

Sistema binario 328

snapshot 430

Sólo lectura (disco) 30

Sonido 348

Sonido en Dr.Logo 255
...
String expression too complex 290

String too long 290

STRING\$ 170

Subscript out of range 289
...
Teclado en Dr.Logo 254

Teclas especiales 32

Temporizadores 267

Terminal Zenith Z19/Z29 310
...
Unexpected RESUME 290

Unexpected RETURN 289
...
variables literales 42

Varias instrucciones en una misma línea de programa 45

Ventanas 340, 342

Verificación de discos 89

Volcado de pantalla 59, 200

Volumen 28

Volumen del sonido 79
...
Welcome (programa de demostración) 38
...
Zócalo de expansión 300

Zócalo del joystick 298

Zócalo del monitor 298

Zócalo estéreo 299

Zócalo para la impresora 301

Zócalo para la segunda unidad de disco 300

Zócalos de entradas y salidas 298
...
En 2008 (durante los meses de octubre a diciembre) se ha realizado una
revisión de este texto, por autores ajenos a los originales. No se
cuenta con permiso escrito de Amstrad, pero, dado que esta empresa dio
permiso para distribuir las ROMs así como otros elementos de software,
hemos dado por sentado que no estamos violando ningún copyright vigente,
y que estamos realizando un servicio a la comunidad de usuarios de
Amstrad CPC.

Los cambios han sido:

-   Revisión de pequeñas erratas en el texto:
    \"adecuadamene\",\"dicos\", \"inviertiendo\", \"máquima\",
...
-   Corrección de algún otro error en el texto. Por ejemplo, \"on break
    stop\" no permite indicar un número de línea.

-   Corrección de algún programa de ejemplo. Por ejemplo en \"Esgrima
    eléctrico\" del apéndice 3 se mostraba el mensaje \"Pulse J/T -
    Joysticks o teclas\" pero se esperaba la pulsación de J o K
    (Keyboard). En el ejemplo de volcado de pantallas, se esperaba la
    pulsación de Y (yes) o N (no).

-   Ampliación de alguna de las listas de \"Palabras clave
    relacionadas\". Por ejemplo, \"closein\" ahora incluye a
    \"closeout\" como relacionada y viceversa, y \"pen\" incluye a
    \"graphics pen\" y a \"ink\".

-   Añadidas varias \"notas del revisor\" para detallar
    incompatibilidades entre modelos de Amstrad CPC, errores existentes
    en fuentes de ejemplo originales y formas de mejorar dichos fuentes.

-   Renumeración de las páginas en forma correlativa.

-   Creación de un índice de contenido mucho más detallado y de un
    índice alfabético también mucho más detallado.

-   Inclusión de capturas de pantalla de los juegos y de algún otro
    concepto intermedio (por ejemplo, el \"cursor de copia\" o las
    formas de dibujar circunferencias).

-   Revisión de los fuentes y publicación con la sintaxis realzada en
    colores para los de mayor complejidad.

-   Añadido un apéndice que habla de los emuladores de Amstrad CPC.

# Apéndice 6 -- Emuladores de Amstrad CPC

Han pasado cerca de 25 años desde el lanzamiento de los Amstrad CPC. Hoy
en día no es fácil encontrarlos en tiendas, pero se puede redescubrir
buena parte de lo que suponía su manejo (salvo la emoción de manejar en
un equipo que en muchos aspectos estaba por delante de sus
contemporáneos) utilizando un emulador, que imite su funcionamiento
desde un ordenador actual.

Además, estos emuladores suelen permitir posibilidades adicionales,
como:
...
-   Lectura de \"imágenes de disco\": ficheros en formato DSK que
    representan el contenido de un diskette de 3 pulgadas de los CPC,
    así como guardado de datos dentro de estos \"discos ficticios\".

-   Depuración de programas paso a paso, pudiendo ver el contenido de la
    memoria y de los registros del procesador Z80.

-   (Y muchas más)

Algunos de los emuladores que se pueden encontrar hoy en día son:

-   WinAPE: Uno de los más actualizados. Para Windows. Está en inglés,
    pero es fácil de usar y funciona bastante bien. Se puede descargar
    de http://www.winape.net/

-   CPCE: Se actualiza casi tanto como WinAPE, pero su configuración es
    algo más difícil, al tener que editar ficheros de texto en vez de
    usar los menús. Para Windows y DOS. Se puede descargar de
    http://cpce.emuunlim.com/

-   JavaCPC: Creado en Java, lo que permite que funcione en distintas
    plataformas, y se pueda usar incluso desde un navegador web. Se
    puede localizar en http://sourceforge.net/projects/javacpc/

-   XCPC: Para sistemas operativos de la familia Unix, como Linux. Su
    página oficial es http://sourceforge.net/projects/xcpc

A modo de ejemplo, la siguiente imagen es la apariencia de WinAPE con un
menú desplegado:

![](media/image92.png){width="3.0716535433070864in"
height="2.319291338582677in"}
